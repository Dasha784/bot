...
async def currency_router(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if amount is None:
        ...
        await send_main_message(user_id, get_text(user_id, 'enter_amount'), ReplyKeyboardRemove())
        return
    # Save currency and go ask for description
    try:
        await state.update_data(currency=code)
        await state.set_state(Form.deal_description.state)
        await send_temp_message(user_id, f"Валюта выбрана: <b>{code}</b>", )
    except Exception as e:
        logger.exception(f"currency_router error: {e}")
    await send_main_message(user_id, get_text(user_id, 'enter_description', amount=amount, currency=code), ReplyKeyboardRemove())
